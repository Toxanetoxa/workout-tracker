...
- Поле `today` считается относительно текущей даты PostgreSQL через `CURRENT_DATE`, поэтому границы суток определяются на стороне базы и не зависят от времени выполнения приложения.
